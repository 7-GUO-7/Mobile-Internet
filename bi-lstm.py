...
data_dir = "txt"   # directory contains text documents
model_size = 2000    # length of output vectors
nb_epochs = 5   # number of training epochs
embedding_size = 300
...
for i in range(MAX_FILE_ID):
    file_name = data_dir + '/' + str(i + 1) + ".txt"
    if os.path.isfile(file_name):
        X.append(load_content(file_name))
print(X)

X_train, X_test, Y_train, Y_test = cross_validation.train_test_split(X, Y,
    test_size=test_ratio, random_state=2020)
